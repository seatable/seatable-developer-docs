...
if [[ "$1" == "-stop" ]]; then
  sudo docker kill seatable-developer-docs
  exit 0
fi

sudo docker build -t seatable-developer-docs .
sudo docker run --name seatable-developer-docs --rm -d -p 8000:8000 -v ${PWD}:/docs seatable-developer-docs
